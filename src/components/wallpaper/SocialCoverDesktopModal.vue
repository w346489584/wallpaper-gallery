...
import WechatMomentsPreview from '@/components/wallpaper/social/WechatMomentsPreview.vue'
...
const DEFAULT_SOCIAL_COVER_AVATAR = 'https://cdn.jsdelivr.net/gh/IT-NuanxinPro/nuanXinProPic@v1.1.13/wallpaper/avatar/动漫/蜡笔小新/%E6%88%B4%E8%80%B3%E6%9C%BA%E9%BA%A6%E5%BD%93%E5%8A%B3%E5%A4%96%E5%8D%96.png'
const DEFAULT_SOCIAL_COVER_NAME = '旧巷听雨'
...
const { collected, liked, toggleCollect, toggleLike } = useInteraction(toRef(props, 'wallpaper'), effectiveSeries)
...
const wechatMomentsPreviewRef = ref(null)

const usageLabel = computed(() => VIDEO_USAGE_SHORT_LABELS[props.wallpaper?.usage] || props.wallpaper?.usage || '动态背景')
...
const viewerName = computed(() => DEFAULT_SOCIAL_COVER_NAME)

const viewerSignature = computed(() => {
  return '生活很苦，自己加糖'
})

const viewerAvatarInitial = computed(() => getAvatarInitial(viewerName.value, '雨'))
const viewerAvatarStyle = computed(() => getAvatarStyle(viewerName.value))
const viewerAvatarSrc = computed(() => DEFAULT_SOCIAL_COVER_AVATAR)
const viewerAvatarSources = computed(() => [DEFAULT_SOCIAL_COVER_AVATAR])
...
function releaseVideoResources() {
  wechatMomentsPreviewRef.value?.release?.()
  resetVideoState()
}
    ...
    <Transition name="modal" @after-leave="onAfterLeave">
      <div v-if="isVisible && wallpaper" class="social-cover-modal">
        <div class="social-cover-modal__content">
          ...
          <div class="social-cover-modal__preview">
            <div v-if="videoError" class="social-cover-modal__preview-error">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div v-else class="social-cover-modal__phone-preview">
              <div class="iphone-frame">
                <div class="screen-container">
                  <div
                    v-if="!videoReady"
                    class="social-cover-modal__loading"
                  >
                    <LoadingSpinner size="md" />
                  </div>

                  <WechatMomentsPreview
                    ref="wechatMomentsPreviewRef"
                    class="social-cover-modal__moments-preview"
                    :class="{ 'is-loading': !videoReady }"
                    :video-src="wallpaper.playbackUrl || wallpaper.url"
                    :cover-poster="wallpaper.thumbnailUrl"
                    :profile-name="viewerName"
                    :profile-signature="viewerSignature"
                    :profile-avatar-src="viewerAvatarSrc"
                    :profile-avatar-sources="viewerAvatarSources"
                    :profile-avatar-initial="viewerAvatarInitial"
                    :profile-avatar-style="viewerAvatarStyle"
          ...
          <div class="social-cover-modal__info">
            ...
            <div class="info-tags">
              ...
              <span class="tag tag--secondary">朋友圈背景</span>
...
.social-cover-modal {
  ...
  background:
    ...
    linear-gradient(135deg, rgba(11, 18, 32, 0.95) 0%, rgba(20, 31, 51, 0.95) 52%, rgba(17, 62, 111, 0.92) 100%);
